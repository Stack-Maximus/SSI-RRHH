-- ============================================================================
-- Corrige: si más de una dimensión sale crítica (nota <= 2), ahora se crea
-- UNA ALERTA POR CADA DIMENSIÓN, no solo una que solo recuerda la peor.
-- Cada una se puede gestionar y cerrar de forma independiente.
-- ============================================================================
...
declare
  ...
  v_clase_nps text;
  v_nota_minima int := 5;
  v_dims_criticas text[] := array[]::text[];
  v_respuesta_id uuid;
  v_alerta boolean := false;
  v_dimensiones_creadas int := 0;
begin
  ...
  for v_key, v_val in select * from jsonb_each(p_notas) loop
    if v_val is not null and v_val::text != 'null' then
      ...
      if v_peso is not null then
        ...
        if v_nota <= 2 then
          v_dims_criticas := array_append(v_dims_criticas, upper(v_key));
        end if;
      end if;
    end if;
  end loop;
  ...
  if v_alerta then
    -- Una alerta por cada dimensión que llegó a nota crítica (1 o 2)
    insert into sat_alertas (respuesta_id, obra, dimension_critica, fecha_alerta, sla_48h, estado)
    select v_respuesta_id, v_obra, sd.nombre, now(), now() + interval '48 hours', 'abierta'
    from sat_dimensiones sd
    where sd.codigo = any(v_dims_criticas);

    get diagnostics v_dimensiones_creadas = row_count;

    -- Si la alerta se disparó solo por ISC bajo o NPS bajo, sin que ninguna
    -- dimensión puntual llegara a 1-2, igual se deja una alerta general.
    if v_dimensiones_creadas = 0 then
      insert into sat_alertas (respuesta_id, obra, dimension_critica, fecha_alerta, sla_48h, estado)
      values (v_respuesta_id, v_obra, 'ISC/NPS general', now(), now() + interval '48 hours', 'abierta');
    end if;
  end if;
